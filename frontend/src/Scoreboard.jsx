...
function Scoreboard({ scoreboard }) {
  ...
  return (
    <Flex justify="space-between" p={4} bg={useColorModeValue('gray.100', 'gray.700')} borderRadius="md">
      <Box>
            ...
            {Object.entries(scoreboard).map(([name, score], index) => (
              ...
              >
                <Flex align="center" justify="space-between">
                  <Text fontWeight="bold" color={textColor}> {/* Use the text color variable */}
                    {name}
                  </Text>
                  <Flex align="center">
                    <Icon as={FaStar} color="yellow.400" mr={1} /> {/* Star icon for visual interest */}
                    <Text>{score}</Text>
                  </Flex>
                </Flex>
              </ListItem>
